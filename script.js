function init() {
  ...
  const triggerBunnyMessage = (bunny, classToAdd) => {
    bunny.el.setAttribute(
      "message",
      ["love!", "Adore!", "In love with!", "^ _ ^", "Have a crush on!"][
        randomN(5) - 1
      ]
    );
  ...
  const hugBunny = (bunny) => {
    ...
    setTimeout(() => {
      ...
      triggerBunnyMessage(
        bunny,
        classToAdd === "hug-bear-bunny" ? "happy-left" : "happy-right"
      );
      updateSadBunnyCount();
    }, 1800);
  };
  ...
  new Array(5).fill("").forEach(() => {
    const bunnyPos = Object.assign(document.createElement("div"), {
      className: "bunny-pos",
    });
  ...
  setInterval(() => {
    ...
    elements.bunnyPos.forEach((indicator, i) => {
      ...
      if (bunny) {
        ...
        indicator.innerHTML = `<div class="bunny-indicator" style="transform: rotate(${
          angle * -1
        }deg)">${distance - 40}px</div>`;
        indicator.style.setProperty(
          "--size",
          px(
            distance > settings.bunnyRadarSize / 2
              ? settings.bunnyRadarSize
              : distance
          )
        );
